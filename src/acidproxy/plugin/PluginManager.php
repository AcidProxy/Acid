...
namespace acidproxy\plugin;

use pocketmine\utils\TextFormat;
use acidproxy\command\PluginCommand;
    ...
    public function loadPlugins(string $directory) {
        ...
        foreach ($dirs as $dir) {
            if (!is_file($file = $dir . $ds . "plugin.yml")) {
                $this->getProxy()->getLogger()->info("Error while loading plugin " . basename($dir) . ": {$file} not found");
                continue;
            }
            $data = yaml_parse_file($dataFile = $dir . $ds . "plugin.yml");
            if (!isset($data['name']) || !isset($data['api']) || !isset($data['version']) || !isset($data['main'])) {
                $this->getProxy()->getLogger()->info("Error while loading plugin " . basename($dir) . ": Invalid plugin description.");
                continue;
            }
            if (!is_file($main = dirname($dataFile) . $ds . "src" . $ds . str_replace("\\", $ds, $data['main']) . ".php")) {
                $this->getProxy()->getLogger()->info("Error while loading plugin " . $data['name'] . ": Main class not found ($main).");
                continue;
            }
            if ($data['api'] !== \acidproxy\VERSION) {
                $this->getProxy()->getLogger()->info("Error while loading plugin " . $data['name'] . ": Incompatible api version.");
                continue;
            }
            ...
            if (!is_a($plugin, PluginBase::class)) {
                $this->getProxy()->getLogger()->info("Could not load plugin " . $data['name'] . ": Invalid main class");
                continue;
            }
        ...
        $this->getProxy()->getLogger()->info("Loaded {$loaded} plugins");
    }
